...
export function conversorController(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        return res.status(200).send(modelConversor)
        ...
        if (error instanceof ExternoApiErro) {
            return res.status(503).json({ error: 'Serviço de cotação indisponível' });
        }
